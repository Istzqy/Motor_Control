...
TIM_HandleTypeDef g_tim15_npwm_handle = {0};
...
void atim_timx_chy_npwm_init(uint16_t arr, uint16_t psc)
{
    ...
    ATIM_TIMX_NPWM_CHY_GPIO_CLK_ENABLE();
		ATIM_TIM15_NPWM_CHY_GPIO_CLK_ENABLE();  
    ATIM_TIMX_NPWM_CLK_ENABLE();
		ATIM_TIM15_NPWM_CLK_ENABLE();
    
    /* 配置PWM输出引脚PA2,PA3,PA8 */
    ...
    HAL_GPIO_Init(ATIM_TIMX_NPWM_CHY_GPIO_PORT, &gpio_init_struct);
		...
		gpio_init_struct.Pin = ATIM_TIM15_NPWM_CHY_GPIO_PIN1|ATIM_TIM15_NPWM_CHY_GPIO_PIN2;
    gpio_init_struct.Mode = GPIO_MODE_AF_PP;
    gpio_init_struct.Pull = GPIO_PULLDOWN;
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;
    gpio_init_struct.Alternate = ATIM_TIM15_NPWM_CHY_GPIO_AF;
    HAL_GPIO_Init(ATIM_TIM15_NPWM_CHY_GPIO_PORT, &gpio_init_struct);
    ...
    HAL_TIM_PWM_Init(&g_timx_npwm_handle);                                                      /* 初始化定时器PWM输出 */
		...
		g_tim15_npwm_handle.Instance = ATIM_TIM15_NPWM;                                               /* 定时器 */
    g_tim15_npwm_handle.Init.Prescaler = psc;                                                    /* 预分频器系数 */
    g_tim15_npwm_handle.Init.CounterMode = TIM_COUNTERMODE_UP;                                   /* 计数模式 */
    g_tim15_npwm_handle.Init.Period = arr;                                                       /* 自动重装载值 */
    g_tim15_npwm_handle.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_ENABLE;                  /* 自动重装载预加载 */
    g_tim15_npwm_handle.Init.RepetitionCounter = 0;                                              /* 重复计数数值 */
    HAL_TIM_PWM_Init(&g_tim15_npwm_handle);                                                      /* 初始化定时器PWM输出 */
		
    
    timx_oc_init_struct.OCMode = TIM_OCMODE_PWM2;                                               	/* 输出比较模式 */
	...
	HAL_NVIC_SetPriority(ATIM_TIM15_NPWM_IRQn, 0, 0);
	HAL_NVIC_SetPriority(ATIM_TIM15_NPWM_IRQn, 0, 0);
...
void atim_tim15_chy_npwm_set(uint32_t npwm)
{
    if (npwm == 0)
    {
        return;
    ...
    /* 产生更新事件来输出指定个数PWM */
    g_timx_chy_npwm_remain = npwm;
    HAL_TIM_GenerateEvent(&g_tim15_npwm_handle, TIM_EVENTSOURCE_UPDATE);
}
...
void ATIM_TIMX_NPWM_IRQHandler(void)
{
    ...
    if (__HAL_TIM_GET_FLAG(&g_timx_npwm_handle, TIM_FLAG_UPDATE) != RESET)
    {
        ...
        if (npwm != 0)
        {
            ...
            g_timx_npwm_handle.Instance->EGR = TIM_EGR_UG;
            __HAL_TIM_ENABLE(&g_timx_npwm_handle);
        }
        else
        {
            ATIM_TIMX_NPWM->CR1 &= ~(TIM_CR1_CEN); /* 重复计数满后关闭计数器 */
        }
...
/**
 * @brief   输出指定个数PWM通用定时器15中断回调函数
 * @param   无
 * @retval  无
 */
void ATIM_TIM15_NPWM_IRQHandler(void)
{
    uint16_t npwm = 0;
    
    /* 定时器更新事件中断 */
    if (__HAL_TIM_GET_FLAG(&g_tim15_npwm_handle, TIM_FLAG_UPDATE) != RESET)
    {
        /* 每次最多输出256个PWM（TIMx_RCR只有[7:0]有效） */
        if (g_timx_chy_npwm_remain > 256)
        {
        ...
        if (npwm != 0)
        {
            /* 利用通用定时器15的重复计数器来输出PWM */
						ATIM_TIM15_NPWM->RCR = npwm - 1;
            g_tim15_npwm_handle.Instance->EGR = TIM_EGR_UG;
            __HAL_TIM_ENABLE(&g_tim15_npwm_handle);
        }
        else
        {
						ATIM_TIM15_NPWM->CR1 &= ~(TIM_CR1_CEN); /* 重复计数满后关闭计数器 */
        }
        
        /* 清除更新事件标志 */
        __HAL_TIM_CLEAR_IT(&g_tim15_npwm_handle, TIM_IT_UPDATE);
    }
}
...
//    
//    timx_oc_init_struct.OCMode = TIM_OCMODE_TOGGLE;                                     /* 输出比较模式 */
//    timx_oc_init_struct.Pulse = (((arr + 1) >> 2) * 2) - 1;                             /* 比较值 */
//    timx_oc_init_struct.OCPolarity = TIM_OCPOLARITY_HIGH;                               /* 输出比较极性 */
...
//    timx_oc_init_struct.OCPolarity = TIM_OCPOLARITY_HIGH;                               /* 输出比较极性 */
//    HAL_TIM_OC_ConfigChannel(&g_timx_comp_handle, &timx_oc_init_struct, TIM_CHANNEL_3); /* 配置输出比较通道 */
//    
//    timx_oc_init_struct.OCMode = TIM_OCMODE_TOGGLE;                                     /* 输出比较模式 */
//    timx_oc_init_struct.Pulse = (((arr + 1) >> 2) * 4) - 1;                             /* 比较值 */
//    timx_oc_init_struct.OCPolarity = TIM_OCPOLARITY_HIGH;                               /* 输出比较极性 */
//    HAL_TIM_OC_ConfigChannel(&g_timx_comp_handle, &timx_oc_init_struct, TIM_CHANNEL_4); /* 配置输出比较通道 */
//    
//    /* 开启输出比较通道输出 */
//    HAL_TIM_OC_Start(&g_timx_comp_handle, TIM_CHANNEL_1);
//    HAL_TIM_OC_Start(&g_timx_comp_handle, TIM_CHANNEL_2);
//    HAL_TIM_OC_Start(&g_timx_comp_handle, TIM_CHANNEL_3);
//    HAL_TIM_OC_Start(&g_timx_comp_handle, TIM_CHANNEL_4);
//}
